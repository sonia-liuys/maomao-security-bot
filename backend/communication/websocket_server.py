...
class WebSocketServer:
    ...
    def _video_streaming_loop(self):
        ...
        camera = None
        try:
            camera = cv2.VideoCapture(0)  # 使用默認攝像頭 / Use default camera
            if not camera.isOpened():
                self.logger.error("無法打開攝像頭，將使用模擬視頻")
                self.logger.error("Cannot open camera, will use simulated video")
                camera = None
            else:
                # 設置攝像頭分辨率
            ...
            try:
                ...
                try:
                    ...
                    if camera and camera.isOpened():
                        ...
                        if not ret:
                            self.logger.error("無法從攝像頭讀取幀，將使用模擬視頻")
                            self.logger.error("Cannot read frame from camera, will use simulated video")
                            # 創建一個模擬幀作為備用
                            ...
                            cv2.putText(frame, f'Camera Error - Simulated Video', (width - 300, height - 20), 
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                    else:
                        # 如果沒有攝像頭，創建一個模擬幀
                        # If no camera, create a simulated frame
                        height, width = 480, 640
                        frame = np.zeros((height, width, 3), dtype=np.uint8)
                        frame[:, :] = (30, 30, 50)  # 藍色背景 / Blue background
